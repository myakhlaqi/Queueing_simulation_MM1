...
public class Simulator {
	...
	/////// metrices////////////
	double avgDelay = 0; // averageDelay is total delay/npd
	...
	public void init() {
		...
		double sum = 0;
		/*
		 * for (int i = 0; i <= 100 * mst; i++) { double iat =
	...
	public String getLog(EventType e) {
		return String.format("%-3d) ;t=%-10.3f;%s-;%-5d;%s", row++, simclock,
...
*/
	...
	private void saveLog(int load, PrintWriter out) {
			...
			String header = String.format("%s\t%-6s\t%-5s\t%5s\t%5s\t%5s\t%4s\t%4s\t" + "%-4s\t%-4s\t%s\t%s\t%-1s",
